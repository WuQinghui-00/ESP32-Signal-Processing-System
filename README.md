...
本项目设计并实现了一套完整的嵌入式信号处理系统，从底层驱动到上层应用，涵盖了**波形生成、数据采集、FFT 频谱分析、WiFi 连接、MQTT 云端上报、Web 实时监控**的全链路闭环。采用 FreeRTOS 任务架构，体现了嵌入式全栈开发能力。
